...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x : Tensor, mask: Tensor = None) -> Tensor:
        ...
        queries = self.relu1(queries)
        keys = self.relu1(keys)
        queries = self.fc1(queries)
        keys = self.fc2(keys)
        # sum up over the last axis
        energy = torch.einsum('bhqd, bhkd -> bhqk', queries, keys) # batch, num_heads, query_len, key_len
        energy = self.relu1(energy)
        if mask is not None:
...
def Cifarmodel(num_classes=10, dropout=0, **kargs):
    return ViT( in_channels = 3,
                patch_size = 4,
                emb_size = 512,
                img_size = 32,
                depth = 8,
