...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        #         Q_spk = self.fc1(Q_spk)
        #         K_spk = self.fc2(K_spk)
        #         Q_spk_rec.append(Q_spk)
        #         K_spk_rec.append(K_spk)
        #     if int(n) <= t < int(2 * n):
        #         att_spk = F.relu((Q_spk_rec[t - int(n)] @ K_spk_rec[t - int(n)].transpose(-2, -1)) * self.scale)
        #         att_spk_rec.append(att_spk)
        # att_spk_rec = torch.stack(att_spk_rec, 0).sum(0)
        # att = att_spk_rec.softmax(dim=-1)
        # V_spk_rec = F.relu(v)
        # x = (att @ V_spk_rec).transpose(1, 2).reshape(B, N, C)
        # x = self.proj(x)
        ...
        # Q_spk = self.fc1(Q_spk)
        # K_spk = self.fc2(K_spk)
        # att_spk = F.relu((Q_spk @ K_spk.transpose(-2, -1)) * self.scale) * 5
        ...
        #     qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        #     # [batch_size, num_heads, num_patches + 1, embed_dim_per_head]
        #     q, k, v = F.relu(qkv[0]), F.relu(qkv[1]), F.relu(qkv[2])
        #
        #
