...
from models.swin import Cifarmodel
sys.path.append('/home/ubuntu/OptSNNConvertion-RNL-RIL')
...
wandb.init(project="test-project", entity="spikingtransformer")
...
sharescale = True
scale_init = 2.5
conf = ['swin','4reluv2',optimizer,str(lr)]
save_name = '_'.join(conf)
log_dir = 'train_' + save_name

if not os.path.isdir(log_dir):
    os.makedirs(log_dir)
train_dataloader, test_dataloader = load_cv_data(data_aug=True,
...
model = Cifarmodel(num_classes[dataset])
...
# weights_path = '/hpc/users/CONNECT/q18010301/ziqing/vision_transformer/train_multi_GPU/weights/swin_4relu_0.915.pth'
# weights_path = '/hpc/users/CONNECT/q18010301/ziqing/vision_transformer/train_multi_GPU/weights/Cifar10_224_1e-05_0.946.pth'
# weights_path = '/hpc/users/CONNECT/q18010301/ziqing/vision_transformer/train_multi_GPU/weights/Cifar10_224_1e-05_0.967.pth'
weights_path = '/hpc/users/CONNECT/q18010301/ziqing/vision_transformer/train_multi_GPU/Cifar10_4reluV2_0.0001/epoch18_0.951.pth'
# weights_path = '/hpc/users/CONNECT/q18010301/ziqing/vision_transformer/train_multi_GPU/Cifar100_4relusv2_0.0001/epoch52_0.819.pth'
...
scheduler = WarmupLR(optimizer=optimizer1, num_warm=10)
# scheduler = optim.lr_sheduler.ReduceLROnPlateau(optimizer1, mode='max', factor=0.5, patience=4,
...
def ann_train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(train_dataloader)):
        ...
        train_batch_cnt += 1
    # scheduler.step(ann_correct / total)
    scheduler.step()
    print('Para Train Epoch %d Loss:%.3f Acc:%.3f' % (epoch,
                                                      ann_train_loss,
                                                      ann_correct / total))
    writer.add_scalar('Train/EpochAcc', ann_correct / total, epoch)
    wandb.log({"train loss": ann_train_loss})
    wandb.log({"train accuracy": ann_correct / total})
    wandb.log({"lr": float(optimizer1.state_dict()['param_groups'][0]['lr'])})
    return
...
def para_train_val(epoch):
    ...
    with torch.no_grad():
        ...
        #                                                      ann_correct / total,
        #                                                      sum_k / cnt_k, last_k))
        print('Test Epoch %d Loss:%.3f Acc:%.3f  LastK:%.3f' % (epoch,
                                                             ann_test_loss,
                                                             ann_correct / total,
                                                              last_k))
        wandb.log({"test loss": ann_test_loss})
        wandb.log({"test accuracy": ann_correct / total})
...
def snn_train(epoch):
    ...
    wandb.log({"fast train accuracy": snn_correct / total})
    wandb.log({"lr": float(optimizer2.state_dict()['param_groups'][0]['lr'])})
...
def snn_val(iter):
    ...
    with torch.no_grad():
        ...
        print('Test Iter %d Loss:%.3f Acc:%.3f AvgK:%.3f LastK:%.3f' % (iter,
                                                                         ...
                                                                         ann_correct / total,
                                                                         sum_k / cnt_k, last_k))
        wandb.log({"fast test accuracy": ann_correct / total})
...
def simulate_by_filename(save_name):
    ...
    model = modules.replace_maxpool2d_by_avgpool2d(model)
    model = modules.replace_relu_by_spikingnorm(model,True)
    state_dict = torch.load(log_dir + '/%s.pth' % save_name)
    ann_acc = state_dict['acc']
    model.load_state_dict(state_dict['net'])
    model = replace_spikingnorm_by_ifnode(model)
    simulate(model.to(device), T=20, save_name='%s' % save_name, log_dir=log_dir, ann_baseline=ann_acc)
...
# model.load_state_dict(torch.load('/hpc/users/CONNECT/q18010301/ziqing/OptSNNConvertion-RNL-RIL/train_swin_4reluv2_adam_0.0001/swin_4reluv2_adam_0.0001.pth')['net'])
...
#                            lr=lr,
#                            weight_decay=decay)
# scheduler2 = WarmupLR(optimizer=optimizer2, num_warm=10)
...
# simulate_by_filename('swin_4reluv2_adam_0.0001_[0.100_95.090_4.706]')
# simulate_by_filename('swin_4reluv2_adam_0.0001_[0.100_95.150_4.768]')
# simulate_by_filename('swin_2relu_adam_0.0001_[0.100_94.700_5.322]')

# simulate_by_filename('Cifarmodel_cifar10_[0.100_85.700_4.742]')




# simulate_by_filename('vgg16_cifar10_[0.100_87.840_8.291]')
